...
// "global state" -- in the real app this uses cookies.
...
export default function App() {
   ...
   */
  function attemptRefreshingAuthToken(error, caught) {
   ...
   */
  function renewRefreshToken(error) {
  ...
  function authenticatedAPICall$(unauthenticatedRequest) {
    return of(unauthenticatedRequest).pipe(
      ...
      catchError(attemptRefreshingAuthToken),
      catchError(renewRefreshToken)
    )
  }

  /**
   * This is in the "service" layer, i.e. *.service.js.
   * It's essentially what the apiCall$ function currently does.
   */
